...
const requireAdmin = async (req, res, next) => {
  ...
  if (!permission) {
    return res.status(403).json({
      message: `Access denied: Sales Data ${action} permission is required`,
    });
  }
...
const datasetVisibilityFilter = (req) =>
  req.user.roleKey === 'super_admin'
    ? {}
    : {
        $or: [
...
const isDatasetOwner = (dataset, userId) =>
  Boolean(
    dataset.uploadedBy && String(dataset.uploadedBy?._id || dataset.uploadedBy) === String(userId),
  );
...
const resolveRequestedBusinessUnit = async (req) => {
  ...
  if (businessUnitId && !mongoose.isValidObjectId(businessUnitId)) {
    return null;
  }
  ...
  const query = businessUnitId
    ...
    : {
        legacyCommunityKey: communityKey,
      };
...
const applyBusinessUnitTableFormat = (communityKey, columns, rows) => {
  ...
  if (!template) {
    return {
      ...addWorkColumnsAfterWebsite(columns, rows),
  ...
  if (missing.length) {
    return {
      error: `${
        communityKey === 'marketing' ? 'Marketing' : 'Live'
...
const getDatasetSalesSummary = (dataset) => {
  ...
  const assignedRows = normalizedData.rows.filter(
    (row, rowIndex) =>
      normalizedAssignments.some((assignment) => Number(assignment.rowIndex) === rowIndex) ||
  ...
  return {
    ...
    openRows: Math.max(
      totalRows - convertedRows - lostRows,
...
const normalizeRowFollowUps = (rowFollowUps = []) =>
  rowFollowUps
    .map((item) => ({
      rowIndex: Number(item.rowIndex),

      followUpDate: normalizeCell(item.followUpDate),
    ...
    .filter((item) => Number.isInteger(item.rowIndex) && item.rowIndex >= 0 && item.followUpDate);

const getFollowUpDatesMap = (rowFollowUps = [], allowedRowIndexes = null) => {
  const allowed = allowedRowIndexes ? new Set(allowedRowIndexes.map(Number)) : null;

  return normalizeRowFollowUps(rowFollowUps).reduce((dates, item) => {
    if (!allowed || allowed.has(Number(item.rowIndex))) {
      dates[String(item.rowIndex)] = item.followUpDate;
    }
...
const getRowMeetingsMap = (meetings = [], allowedRowIndexes = null) => {
  const allowed = allowedRowIndexes ? new Set(allowedRowIndexes.map(Number)) : null;

  return meetings.reduce((meetingMap, meeting) => {
    const meetingObject = typeof meeting.toObject === 'function' ? meeting.toObject() : meeting;

    const rowIndex = Number(meetingObject.rowIndex);

    if (!Number.isInteger(rowIndex) || rowIndex < 0 || (allowed && !allowed.has(rowIndex))) {
      return meetingMap;
    }

    const key = String(rowIndex);

    meetingMap[key] = [...(meetingMap[key] || []), meetingObject];

    return meetingMap;
  }, {});
};

const prepareDatasetResponse = (dataset, includeLogs = false, meetings = []) => {
...
const getEmployeeDatasetResponse = (dataset, employeeId, meetings = []) => {
  ...
  const assignedRowIndexSet = new Set(assignedRowIndexes);

  const visibleAssignments = normalizeAssignments(datasetObject.rowAssignments || []).filter(
    (assignment) => assignedRowIndexSet.has(Number(assignment.rowIndex)),
  );

  const visibleRowLogs = normalizeRowLogs(datasetObject.rowLogs || []).filter((rowLog) =>
    assignedRowIndexSet.has(Number(rowLog.rowIndex)),
  );
  ...
  return {
    ...
    communityKey: datasetObject.communityKey,

    label: datasetObject.label,

    priority: datasetObject.priority,

    source: datasetObject.source,

    ownerAlias: datasetObject.ownerAlias,

    salesStage: datasetObject.salesStage,

    uploadedBy: datasetObject.uploadedBy,
    ...
    rowAssignments: visibleAssignments,

    rowLogs: visibleRowLogs,

    followUpDates: getFollowUpDatesMap(
      datasetObject.rowFollowUps || [],

      assignedRowIndexes,
    ),

    rowMeetings: getRowMeetingsMap(
      meetings,
    ...
    summary: getDatasetSalesSummary({
      columns: normalizedData.columns,

      rows: assignedRows,

      rowAssignments: visibleAssignments.map((assignment) => ({
        ...assignment,

        rowIndex: assignedRowIndexes.indexOf(Number(assignment.rowIndex)),
      })),
    }),
    ...
    const businessUnitQuery =
      req.user.roleKey === 'super_admin'
        ...
        : {
            ...
            legacyCommunityKey: {
              $in: req.datasetCommunityKeys || req.user.communities || [],
            },
          };
...
router.get('/assigned/me', authMiddleware, async (req, res) => {
  try {
    ...
    const assignedDatasets = datasets.map((dataset) => {
      const response = isDatasetOwner(dataset, req.user.id)
        ? prepareDatasetResponse(dataset, false)
        : getEmployeeDatasetResponse(dataset, req.user.id);
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (!viewPermission) {
      return res.status(403).json({
        message: 'Access denied: Sales Data view permission is required',
      });
    }
    ...
    if (!dataset) {
      return res.status(404).json({
        message: 'Client dataset not found',
      });
    }
    ...
    const meetings = await Meeting.find({
      ...
      .select('dataset rowIndex meetingDate meetingTime status')
      .sort({
        meetingDate: 1,
    ...
    if (!hasFullDatasetAccess) {
      ...
      if (assignedDataset.rows.length === 0) {
        return res.status(403).json({
          message: 'No assigned data found in this dataset',
        });
      }
...
router.patch('/:id/rows/:rowIndex/status', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    const requestedFollowUpDate = normalizeCell(followUpDate);
    ...
    if (req.user.roleKey !== 'super_admin' && !ownsDataset) {
      ...
      if (
        ...
        return res.status(403).json({
          message: 'You can update only your assigned data',
        });
      }
    }
    ...
    const existingFollowUps = normalizeRowFollowUps(dataset.rowFollowUps || []);

    const previousFollowUp = existingFollowUps.find(
      (item) => Number(item.rowIndex) === Number(rowIndex),
    );

    const previousFollowUpDate = previousFollowUp?.followUpDate || '';

    const currentFollowUpDate = currentStatus === 'Follow Up' ? requestedFollowUpDate : '';

    const followUpDateChanged = previousFollowUpDate !== currentFollowUpDate;
    ...
    if (statusChanged || remarkChanged || followUpDateChanged) {
      ...
      if (employeeIndex !== -1) {
        updatedRow[employeeIndex] = actorName;
      }
    }

    if (followUpDateChanged) {
      const remainingFollowUps = existingFollowUps.filter(
        (item) => Number(item.rowIndex) !== Number(rowIndex),
      );
      ...
      dataset.rowFollowUps = remainingFollowUps;

      dataset.markModified('rowFollowUps');
    }
    ...
    if (statusChanged || remarkChanged || followUpDateChanged) {
      ...
      const nextLogState = upsertRowLogEntry(
        dataset.rowLogs || [],
    ...
    if (
      (statusChanged || remarkChanged || followUpDateChanged) &&
      ...
      const changes = [
        ...
        followUpDateChanged
          ? currentFollowUpDate
            ? `follow-up date: ${currentFollowUpDate}`
    ...
    const responsePayload = {
      ...
      followUpDates: getFollowUpDatesMap(dataset.rowFollowUps || []),
    };

    responsePayload.rowLog = updatedRowLog || {
      rowIndex,
...
router.patch('/:id/assign', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    if (!allowedModes.includes(assignmentMode)) {
      return res.status(400).json({
        message: 'Select a valid assignment mode',
      });
    }
    ...
    if (unauthorizedEmployees.length) {
      return res.status(403).json({
        message: `${unauthorizedEmployees
          .map((employee) => employee.name || employee.email)
          .join(', ')} cannot access this dataset Business Unit`,
      });
    }
    ...
    if (assignmentMode === 'full') {
      rowIndexes = availableRows;
    }

    if (assignmentMode === 'half') {
      rowIndexes = availableRows.slice(
        0,

        Math.ceil(availableRows.length / 2),
      );
    }
    ...
    if (assignmentMode === 'limited') {
      ...
      if (!Number.isInteger(limit) || limit < 1) {
        return res.status(400).json({
          message: 'Enter a valid record limit',
        });
      }

      if (limit > availableRows.length) {
        return res.status(400).json({
          message: `Only ${availableRows.length} unassigned records are available`,
        });
      }
    ...
    if (!assignedCount) {
      return res.status(409).json({
        message: 'Selected data is already assigned to the selected employees',
      });
    }
...
router.patch('/:id/unassign', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    if (rowIndexes.length === 0) {
      return res.status(400).json({
        message: 'Select at least one row to unassign',
      });
    }

    if (employeeIds.some((employeeId) => !mongoose.isValidObjectId(employeeId))) {
      return res.status(400).json({
        message: 'Select valid employees to unassign',
      });
    }
    ...
    const shouldRemoveAssignment = (assignment) => {
      const rowMatches = rowIndexes.includes(Number(assignment.rowIndex));
      ...
      return employeeIdSet.has(String(assignment.employee));
    };

    const removedAssignments = currentAssignments.filter(shouldRemoveAssignment);

    if (!removedAssignments.length) {
      return res.status(409).json({
        message: employeeIdSet.size
          ? 'Selected employee is not assigned to the selected client rows'
          : 'Selected client rows are already unassigned',
      });
    }
    ...
    return res.json({
      message: employeeIdSet.size
        ? `${removedAssignments.length} employee assignment${removedAssignments.length === 1 ? '' : 's'} removed`
        : `${removedAssignments.length} assignment${removedAssignments.length === 1 ? '' : 's'} removed from selected rows`,

      removedCount: removedAssignments.length,

      removedEmployeeIds: [...removedByEmployee.keys()],
...
router.patch('/:id', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    if (ownerAlias) {
      dataset.ownerAlias = ownerAlias;
    }

    if (salesStage) {
      dataset.salesStage = salesStage;
    }
...
router.patch('/labels/bulk', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    if (datasetIds.length === 0) {
      return res.status(400).json({
        message: 'Select at least one account list',
      });
    }

    if (!label && !priority && !salesStage) {
      return res.status(400).json({
        message: 'Add a label, priority, or sales stage to update',
      });
    }
...
router.post('/', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    if (businessUnit === false) {
      return res.status(403).json({
        message: 'Business Unit access denied',
      });
    }

    if (!businessUnit) {
      return res.status(400).json({
        message: 'Select a valid Business Unit',
      });
    }
...
router.post('/upload', authMiddleware, requireAdmin, upload.single('file'), async (req, res) => {
  try {
    ...
    if (!headerRow) {
      return res.status(400).json({
        message: 'No header row found in the Excel file',
      });
    }
